...
#include "Controller/AvatarCRAMController.h"
#include "Engine.h"

void UAvatarCRAMController::Init(AActor* InAgent)
{
	if (!InAgent)
	{
		UE_LOG(LogTemp, Error, TEXT("Avatar CRAM Controller not attached to an Avatar"));
	}
...
UAvatarCRAMController::UAvatarCRAMController()
{
	bAnswering = false;
	CSVLine= "";
...
void UAvatarCRAMController::Tick(float InDeltaTime)
{
	CancelAction();
	if (bActive)
	{
...
void UAvatarCRAMController::NotifyQuestion()
{
...
void UAvatarCRAMController::CheckAnswer()
{
	FString AnswerText;
	int8 decision;
...
void UAvatarCRAMController::GiveAnswer(FString InAnswer)
{
...
void UAvatarCRAMController::TimeOut()
{
